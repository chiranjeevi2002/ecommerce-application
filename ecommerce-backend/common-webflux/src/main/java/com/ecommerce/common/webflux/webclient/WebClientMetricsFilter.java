...
public final class WebClientMetricsFilter {
    ...
    public ExchangeFilterFunction filter() {
        ...
        return (request, next) -> {
            ...
            Mono<ClientResponse> exchangeMono =
                    next.exchange(request);

            return exchangeMono
                    .doOnSuccess(response ->
                            recordSuccess(request, response, startTime)
                    )
                    .doOnError(error ->
                            recordFailure(request, startTime)
                    );
        };
    }
    ...
    private void recordSuccess(
        ...
        Timer.builder("http.client.requests")
                ...
                .tag("uri", normalizeUri(request))
                .tag("status", String.valueOf(response.statusCode().value()))
                .tag("service", extractServiceName(request))
