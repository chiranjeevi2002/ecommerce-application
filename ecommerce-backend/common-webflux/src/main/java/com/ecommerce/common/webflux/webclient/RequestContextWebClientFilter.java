...
public final class RequestContextWebClientFilter {
	...
	public static ExchangeFilterFunction propagateContext() {
		...
		return (request, next) -> ReactiveContextHolder.get().defaultIfEmpty(null)
				.flatMap(ctx -> next.exchange(enrichRequest(request, ctx)));
	}

	private static ClientRequest enrichRequest(ClientRequest request, ReactiveRequestContext ctx) {
		...
		if (ctx.roles() != null && !ctx.roles().isEmpty()) {
			builder.header(RequestHeaders.USER_ROLES, String.join(",", ctx.roles()));
		}
